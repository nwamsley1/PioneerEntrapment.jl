...
"""
    add_protein_efdr_columns!(protein_results; method_types=[CombinedEFDR,PairedEFDR], score_qval_pairs=[(:pg_score,:qval)], r=1.0, max_evaluation_points=10_000, use_fast_paired=true, entrap_labels_override=nothing) -> Nothing
...
function add_protein_efdr_columns!(protein_results::DataFrame;
    ...
    for (score_col, qval_col) in score_qval_pairs
        ...
        n_proteins = length(scores)
        adaptive_stride = max(1, ceil(Int, n_proteins / max_evaluation_points))
        ...
        for method_type in method_types
            ...
            efdr_values = if method_type == PairedEFDR
                if use_fast_paired
                    ...
                    @info "PairedEFDR ($score_col): Using strided O(n²) implementation with adaptive_stride=$adaptive_stride (n_proteins=$n_proteins, max_evaluation_points=$max_evaluation_points)"
